const getState = (plate) => {
    ...
    // const RS = /^((IA[Q-Z])|(J[A-C][A-Z])|(JD[A-O]))/
    // const SC = 
    // [
    //     /^((LW[R-Z])|(M[A-L][A-Z])|(MM[A-M]))/,
    //     /^(OK[D-H])/,
    //     /^(Q[H-J][A-Z])/,
    //     /^(QT[K-M])/,
    //     /^(RA[A-J])/,
    ...
    // if (RS.test(plate)) return "Rio Grande do Sul"

    // for (let i = 0; i < PR.length; i++) {
    //     if (PR[i].test(plate)) return "Paraná"
    // }

    // for (let i = 0; i < SC.length; i++) {
    //     if (SC[i].test(plate)) return "Santa Catarina"
    // }
...
window.addEventListener('keyup', (e) => {
    if (e.key === 'Enter') {
        ...
        try {
            ...
            output.innerHTML = state == 0 ? "Sequência ainda não foi atribuída à nenhum estado" : `Placa: ${inputValue}<br>Estado: ${state}`

        } catch (e) {
            output.innerHTML = 'Placa inválida. Informe uma nova placa no formato (L L L Nº L Nº Nº)'
